...
public class BeFanPom extends BasePageFw {
	...
	@FindAll({ @FindBy(css = "[class*='tpt-nav'] [class='dropdown']") })
	private WebElement dropdown_toggle;

	@FindAll({ @FindBy(css = "[class=col-lg-2] select") })
	private List<WebElement> elementosDesplegablesGestion;

	@FindAll({ @FindBy(css = "[class='dropdown open'] ul div div ul li ") })
	...
	public BeFanPom(WebDriver driver) {
		...
		super.fluentWait.withTimeout(30, TimeUnit.SECONDS).pollingEvery(5, TimeUnit.SECONDS)
				.ignoring(NoSuchElementException.class).ignoring(NullPointerException.class)
				.ignoring(TimeoutException.class).ignoring(ElementNotVisibleException.class);
	}
	...
	public void selectOpcion(WebElement desplegable, String nombreOpcionVisible) {
		// recibe un WebElement que lo pasa a Select y selecciona la opcion de acuerdo
		// al String que debe coincidir con el texto Visible <-(ojo con esto)
	...
	public void setFechaHasta_Gestion(String dia, String mes, String ano) {
		// String fecha=dia+'/'+mes+'/'+ano;<---------------------- Esto no Funciona ya
		// que si se edita los valores solo por JS luego no busca la gestion
